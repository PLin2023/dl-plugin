...
const initialFormState: ProductCategory = {
  metadata: {
    name: "",
    generateName: "product-category-",
  },
  spec: {
    name: "",
    description: "",
  ...
  apiVersion: "productCategory.plugin.valve.zpl/v1alpha1",
};

const formState = ref<ProductCategory>(cloneDeep(initialFormState));
...
const modalTitle = computed(() => {
  return isUpdateMode.value ? "编辑产品类别" : "新建产品类别";
});
...
const handleSaveCategory = async () => {
  ...
  try {
    ...
    if (isUpdateMode.value) {
      await axiosInstance.put<ProductCategory>(
        `/apis/productCategory.plugin.valve.zpl/v1alpha1/productCategorys/${formState.value.metadata.name}`,
        formState.value
      );
    } else {
      await axiosInstance.post<ProductCategory>(`/apis/productCategory.plugin.valve.zpl/v1alpha1/productCategorys`, formState.value);
    }
...
<template>
  <VModal :title="modalTitle" :visible="visible" :width="650" @update:visible="onVisibleChange">
    ...
    <div class="md:grid md:grid-cols-4 md:gap-6">
      ...
      <div class="mt-5 md:col-span-3 md:mt-0 divide-y divide-gray-100">
        <AnnotationsForm
          ...
          kind="ProductCategory"
          group="productCategory.plugin.valve.zpl"
        />
      </div>
    </div>
    ...
    <template #footer>
      <VSpace>
        <VButton :loading="saving" type="secondary" @click="submitForm('product-category-form')">提交</VButton>
